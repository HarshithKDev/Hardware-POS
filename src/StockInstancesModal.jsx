              ...
              {Object.values(instances.filter(i => i.is_active && (!filterBatchId || i.batch_id === filterBatchId)).reduce((acc, inst) => {
                ...
                <div key={`${group.original_length}_${group.current_length}_${group.location}`} className="flex flex-wrap md:flex-nowrap items-center justify-between p-3 rounded-lg bg-[var(--bg-primary)] border border-[var(--border-light)] shadow-sm">
                  <div className="flex items-center gap-6 w-full md:w-auto overflow-x-auto hide-x-scrollbar">
                    ...
                    <div className="pl-4 border-l border-[var(--border-light)]">
                      <div className="text-[9px] font-bold uppercase tracking-widest text-[var(--text-tertiary)] mb-0.5">Location</div>
                      <div className="text-xs font-bold" style={{ color: String(group.location || '').toLowerCase() === 'store' ? 'var(--color-success)' : 'var(--color-accent)' }}>
                        {String(group.location || 'Store').toUpperCase()}
                      </div>
                    </div>
                  </div>
                  
                  <div className="flex gap-2 justify-end shrink-0 mt-3 md:mt-0 ml-4">
                    <button 
                      onClick={() => handlePrintClick(group)} 
                      className="px-3 py-1.5 rounded-md transition-colors font-bold text-[10px] uppercase tracking-wider flex items-center gap-1.5 border border-[var(--color-accent)] text-[var(--color-accent)] hover:bg-[var(--color-accent-bg)]"
                      title="Print Barcode"
                    >
                    ...
                    <button 
                      ...
                      className="px-3 py-1.5 rounded-md transition-colors font-bold text-[10px] uppercase tracking-wider flex items-center gap-1.5 border border-red-500 text-red-500 hover:bg-red-500/10"
                      title="Discard Piece"
                    >
