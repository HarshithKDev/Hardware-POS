...
function App() {
  ...
  if (isInitialLoad) {
    return (
      <div className="w-full min-h-screen bg-[#f3f3f3] flex flex-col items-center justify-center text-black">
        <Spinner className="w-10 h-10 text-[#0078D7] mb-4" />
        <p className="text-sm font-semibold uppercase text-gray-600 tracking-wider">Starting System</p>
      </div>
    );
  }
  ...
  const displayUserName = userRole === 'owner' ? (shopSettings?.owner_name || 'Admin') : 'Staff Access';

  return (
    <div className="w-full min-h-screen bg-[#f3f3f3] text-black font-sans relative">
        ...
        <div className="fixed inset-0 bg-black/50 flex items-center justify-center z-[100] px-4 print:hidden">
          <div className="bg-white border border-gray-400 w-[400px] shadow-[0_4px_12px_rgba(0,0,0,0.15)] flex flex-col">
            <div className="bg-white flex justify-between items-center pr-1 pl-4 py-1 border-b border-gray-200">
              ...
              <button onClick={() => setShowLogoutConfirm(false)} className="text-gray-600 hover:bg-[#e81123] hover:text-white px-3 py-1.5 leading-none transition-none focus:outline-none">✕</button>
            </div>
            ...
            <div className="p-4 bg-[#f3f3f3] border-t border-gray-300 flex justify-end gap-2">
              <button onClick={confirmLogout} className="px-6 py-1.5 bg-[#0078D7] hover:bg-[#005a9e] text-white text-sm border border-transparent focus:outline-none focus:ring-1 focus:ring-black">Sign Out</button>
              <button onClick={() => setShowLogoutConfirm(false)} className="px-6 py-1.5 bg-[#e6e6e6] hover:bg-[#cccccc] text-black border border-gray-400 text-sm focus:outline-none focus:border-[#0078D7]">Cancel</button>
            </div>
          </div>
        </div>
      )}
      ...
      {isMobileScannerOpen && (
        <div className="fixed inset-0 bg-black/50 flex items-center justify-center z-[110] px-4 print:hidden">
          <div className="bg-white w-full max-w-[450px] border border-gray-400 shadow-[0_4px_12px_rgba(0,0,0,0.15)] flex flex-col">
            <div className="bg-white flex justify-between items-center pr-1 pl-4 py-1 border-b border-gray-200">
              <span className="text-xs font-semibold text-black">Mobile Scanner</span>
              <button onClick={() => setIsMobileScannerOpen(false)} className="text-gray-600 hover:bg-[#e81123] hover:text-white px-3 py-1.5 leading-none transition-none focus:outline-none">✕</button>
            </div>
            <div className="p-6 flex flex-col items-center bg-white">
              <div id="reader" className="w-full bg-white border border-gray-300"></div>
            ...
            <div className="p-4 bg-[#f3f3f3] border-t border-gray-300 flex justify-end">
              <button onClick={() => setIsMobileScannerOpen(false)} className="px-6 py-1.5 bg-[#e6e6e6] hover:bg-[#cccccc] text-black border border-gray-400 text-sm w-full md:w-auto focus:outline-none focus:border-[#0078D7]">Close</button>
            </div>
          </div>
        </div>
      )}
        ...
        <div className="fixed inset-0 bg-black/50 flex items-center justify-center z-[120] px-4 print:hidden">
          <div className="bg-white border border-gray-400 w-[400px] shadow-[0_4px_12px_rgba(0,0,0,0.15)] flex flex-col">
            <div className="bg-white flex justify-between items-center pr-1 pl-4 py-1 border-b border-gray-200">
              ...
              <button onClick={() => setScannedProduct(null)} className="text-gray-600 hover:bg-[#e81123] hover:text-white px-3 py-1.5 leading-none transition-none focus:outline-none">✕</button>
            </div>
            <div className="p-6 bg-white">
              ...
              <div className="flex gap-4">
                <div className="flex-1 bg-[#f3f3f3] p-4 border border-gray-300 text-center">
                ...
                <div className="flex-1 bg-white p-4 border-2 border-[#0078D7] text-center">
            ...
            <div className="p-4 bg-[#f3f3f3] border-t border-gray-300 flex justify-end">
              <button onClick={() => setScannedProduct(null)} className="px-6 py-1.5 bg-[#0078D7] hover:bg-[#005a9e] text-white text-sm border border-transparent focus:outline-none focus:ring-1 focus:ring-black">OK</button>
            </div>
          </div>
        </div>
      )}
      ...
      <nav className="bg-white text-black border-b border-gray-300 px-4 py-2 flex justify-between items-center print:hidden shadow-sm">
        <div className="flex items-center">
          ...
          <div className="flex gap-1">
            <button onClick={() => setIsMobileScannerOpen(true)} className="md:hidden px-3 py-1.5 bg-transparent border border-gray-300 hover:bg-gray-100 text-xs text-black focus:outline-none focus:border-[#0078D7]">Scan</button>
            {userRole === 'owner' ? (
              <>
                <button onClick={() => setCurrentScreen('dashboard')} className={`px-4 py-1.5 text-xs focus:outline-none ${currentScreen === 'dashboard' ? 'bg-[#0078D7] text-white border border-[#0078D7]' : 'bg-transparent border border-transparent hover:bg-gray-100 text-black'}`}>Management</button>
                <button onClick={() => setCurrentScreen('printer')} className={`px-4 py-1.5 text-xs focus:outline-none ${currentScreen === 'printer' ? 'bg-[#0078D7] text-white border border-[#0078D7]' : 'bg-transparent border border-transparent hover:bg-gray-100 text-black'}`}>Barcodes</button>
              </>
            ) : (
              <button onClick={() => setCurrentScreen('billing')} className={`px-4 py-1.5 text-xs focus:outline-none ${currentScreen === 'billing' ? 'bg-[#0078D7] text-white border border-[#0078D7]' : 'bg-transparent border border-transparent hover:bg-gray-100 text-black'}`}>Terminal</button>
            )}
          </div>
        </div>
        <button onClick={() => setShowLogoutConfirm(true)} className="px-4 py-1.5 bg-transparent hover:bg-[#e81123] hover:text-white text-xs text-black border border-transparent transition-none">Sign Out</button>
      </nav>
