...
export function MobileScannerModal({ onClose, setScannedProduct }) {
  // Use a ref so the scanner instance persists across re-renders without
  ...
  useEffect(() => {
    ...
    scannerRef.current.render(async (decodedText) => {
      ...
      if (data) {
        ...
        // Clear before closing so the camera shuts down cleanly
        scannerRef.current?.clear().catch(() => {});
        onClose();
      } else {
        // Use console.warn instead of alert() — the parent can surface this via its own modal
  ...
  // onClose and setScannedProduct are stable refs passed from App — safe to list
    ...
    <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-[500] px-4 print:hidden">
      <div className="bg-white w-full max-w-[450px] border border-gray-400 shadow-[0_4px_12px_rgba(0,0,0,0.15)] flex flex-col rounded-none">
        ...
        <div className="p-6 flex flex-col items-center bg-white">
          <div id="reader" className="w-full bg-white border border-gray-300 rounded-none" />
